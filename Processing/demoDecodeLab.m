...
for ff = 1 : numel(fn)
    ...
    for ctx = PAR : PAR
        ...
        % **** create data structure to collect all spikes (for processing.org) ****
        allSpikes = [];
        ...
        for bin = startbin : numbins
            ...
            for tt = 1 : ntrials
                if (decParams.binwindow == 1)
                    allSpikesRow = ntrials*(bin-1)+tt;
                    these_spikes(tt, :) = bindata(tt : ntrials : end, firstBinCol + bin - 1)';
                    allSpikes(allSpikesRow,3:2+size(decoding_cells,1)) = bindata(tt : ntrials : end, firstBinCol + bin - 1)';
                    allSpikes(allSpikesRow,1) = decvar(tt);
                    allSpikes(allSpikesRow,2) = bin;
                else
...
% put allSpikes into a text file for Weka or for processing.org
fID = fopen('allSpikes.txt', 'w');
fprintf(fID,'class, time');
for cell = decoding_cells
    fprintf(fID,', %d',cell);
end;
fprintf(fID,'\n');
